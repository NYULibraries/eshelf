...
describe Primo do
  ...
  describe '#becomes_external_system', :vcr do
    ...
    context 'when record has special characters' do
      before do
        @record_with_special_chars = Record.new(external_system: 'primo', external_id: 'nyu_aleph003376830').becomes_external_system
        @record_with_special_chars.save
      end
      subject { @record_with_special_chars }
      its(:author) { is_expected.to eql 'Pedró, Francesc; Programme for International Student Assessment; Centre for Educational Research and Innovation; Organisation for Economic Co-operation and Development; SourceOECD (Online service)' }
    end
    ...
    describe '#locations' do
      ...
      its(:'last.collection') { is_expected.to eql 'New School Fogelman Library Main Collection' }
